...
namespace ET
{
    ...
    {
        protected override void Destroy(AOIUnitComponent self)
        {
    ...
    public static class AOIUnitComponentSystem
    {
        ...
        public static void RefreshUnit(this AOIUnitComponent self)
        {
            if (self.Type == UnitType.Player)
            {
                ...
                EventSystem.Instance.Publish(self.DomainScene(),new AOIRegisterUnit(){Receive = self,Units = units});
                units.Dispose();
            }
        }
        ...
        public static void ChangeTo(this AOIUnitComponent self,AOICell newgrid)
        {
            ...
            using (DictionaryComponent<AOIUnitComponent, int> dic = DictionaryComponent<AOIUnitComponent, int>.Create())
            {
                ...
                foreach (var item in dic)
                {
                    if (item.Value > 0)
                    {
                        EventSystem.Instance.Publish(self.DomainScene(),new EventType.AOIRegisterUnit()
                        {
                            Receive = item.Key,
                            Units = list,
                        });
                    ...
                    {
                        EventSystem.Instance.Publish(self.DomainScene(),new EventType.AOIRemoveUnit()
                        {
                            Receive = item.Key,
                            Units = list
                        });
