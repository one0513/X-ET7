...
namespace ET
{
    ...
    [ObjectSystem]
    [FriendOf(typeof(AOICell))]
    public class AOITriggerDestroySystem : DestroySystem<AOITrigger>
    {
        protected override void Destroy(AOITrigger self)
        {
    ...
    public static class AOITriggerSystem
    {
        ...
        /// <param name="self"></param>
        /// <returns></returns>
        public static float3 GetRealPos(this AOITrigger self)
        {
            if (self.OffsetY != 0)
            {
        ...
        /// <returns></returns>
        public static float3 GetRealPos(this AOITrigger self, float3 pos)
        {
            if (self.OffsetY != 0)
            {
                return pos + new float3(0, self.OffsetY, 0);
            }
            else
            {
                return pos;
        ...
        /// <param name="scale">长宽高</param>
        /// <returns></returns>
        public static AOITrigger AddOBBCollider(this AOIUnitComponent self, float3 scale)
        {
            if (self.Collider != null)
            {
                Log.Error("添加Collider时，Collider已存在");
        ...
        /// <param name="scale">长宽高</param>
        /// <returns></returns>
        public static AOITrigger AddOBBColliderWithoutBroadcast(this AOIUnitComponent self, float3 scale)
        {
            if (self.Collider != null)
            {
                Log.Error("添加Collider时，Collider已存在");
        ...
        public static void RemoveTriggerListener(this AOIUnitComponent self, AOITrigger trigger)
        {
            ...
            var len = self.Scene.gridLen;
            int count = (int)MathF.Ceiling(trigger.Radius / len);
            if (count > 2) Log.Info("检测范围超过2格，触发半径："+ trigger.Radius);
        ...
        public static void AfterTriggerChangeBroadcastToMe(this AOITrigger self, float3 beforePosition,
            bool changeCell)
        {
            if (self.IsCollider) return;
            var unit = self.GetParent<AOIUnitComponent>();
            var len = unit.Scene.gridLen;
            int count = (int) MathF.Ceiling(self.Radius / len);
        ...
        /// <param name="changeCell">是否跨格子</param>
        public static void AfterColliderChangeBroadcastToOther(this AOITrigger self,float3 beforePosition,quaternion beforeRotation,bool changeCell)
        {
            if(!self.IsCollider) return;
            var unit = self.GetParent<AOIUnitComponent>();
            HashSetComponent<AOITrigger> pre; //之前有的
            ...
            else
            {
                pre = HashSetComponent<AOITrigger>.Create(); //之前有的
                after = HashSetComponent<AOITrigger>.Create(); //现在有的
                int count = (int) MathF.Ceiling(self.Radius / unit.Scene.gridLen);
                if (count > 2) Log.Info("检测范围超过2格，触发半径：" + self.Radius);
                DictionaryComponent<AOICell, int> triggers = DictionaryComponent<AOICell, int>.Create();
                for (int i = 0; i < self.FollowCell.Count; i++)
                {
            ...
            float3 position1, quaternion rotation1, float3 position2, quaternion rotation2)
        {
            ...
            var sqrDis = (pos1- pos2).SqrMagnitude();
            ...
            else if(trigger1.TriggerType <= TriggerShapeType.Cube && trigger2.TriggerType <= TriggerShapeType.Cube)//判断OBB和球触发
            {
                ...
                if (-xMax <= temp.x && temp.x <= xMax)
                {
                    ...
                    {
                        return (temp- new float3(temp.x, -yMax, -zMax)).SqrMagnitude() <= triggerSphere.SqrRadius;
                    }
                ...
                if (-yMax <= temp.y && temp.y <= yMax)
                {
                    ...
                    {
                        return (temp- new float3(-xMax, temp.y, -zMax)).SqrMagnitude() <= triggerSphere.SqrRadius;
                    }
                ...
                if (-zMax <= temp.z && temp.z <= zMax)
                {
                    ...
                    {
                        return (temp- new float3(-xMax, -yMax, temp.z)).SqrMagnitude() <= triggerSphere.SqrRadius;
                    }
                ...
                {
                    if ((temp- points[i]).SqrMagnitude() > triggerSphere.SqrRadius)
                    {
                        return false;
                    }
                }
        ...
        {
            var sqrDis = (center- position).SqrMagnitude();
        ...
        {
            //求点到直线的距离
            var dis = Math.Sqrt(math.cross(ray.Start - center, ray.Dir).SqrMagnitude() / ray.Dir.SqrMagnitude());
            ...
            var hit = ray.Start + (float)(Math.Sqrt(disHitStartSqr)-Math.Sqrt(trigger.Radius * trigger.Radius - dis * dis))
                *ray.Dir;
            if (ray.Distance * ray.Distance< disHitStartSqr||(hit- center).SqrMagnitude() - trigger.SqrRadius > 0.1)
            {
        ...
        public static bool IsRayInTrigger(this AOITrigger trigger,Ray ray, float3 center,quaternion rotation,out float3 hit)
        {
            ...
            hit = ray.Start + (float)(Math.Sqrt(disHitStartSqr)-Math.Sqrt(trigger.Radius * trigger.Radius - dis * dis))
                *ray.Dir;
            if (ray.Distance * ray.Distance< disHitStartSqr||(hit- center).SqrMagnitude() > trigger.SqrRadius)
            {
                return false;//即使距离小于半径，由于是射线也要判断一下
            }
            if (trigger.TriggerType == TriggerShapeType.Cube)
