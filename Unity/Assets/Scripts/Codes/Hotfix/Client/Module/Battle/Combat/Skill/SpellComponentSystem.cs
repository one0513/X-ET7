...
namespace ET
{
    [Invoke(TimerInvokeType.PlayNextSkillStep)]
    [FriendOf(typeof(SpellComponent))]
    public class PlayNextSkillStep: ATimer<SpellComponent>
    {
        protected override void Run(SpellComponent self)
        {
            try
            {
    ...
    public static class SpellComponentSystem
    {
        ...
        public static void SpellWithTarget(this SpellComponent self, SkillAbility spellSkill, CombatUnitComponent targetEntity)
        {
                ...
                spellSkill.SkillConfig.PreviewRange[0] > 0)//不填或者填非正数表示无限距离
            {
                ...
                if (math.distance(new float2(nowpos.x, nowpos.z),new float2(nowpos2.x, nowpos2.z)) >
        ...
        public static void SpellWithPoint(this SpellComponent self,SkillAbility spellSkill, float3 point)
        {
            ...
            self.CurSkillConfigId = spellSkill.ConfigId;
            var nowpos = self.GetParent<CombatUnitComponent>().unit.Position;
            if (math.distance(new float2(nowpos.x, nowpos.z), new float2(point.x, point.z)) >
                spellSkill.SkillConfig.PreviewRange[0])
            {
        ...
        public static void SpellWithDirect(this SpellComponent self,SkillAbility spellSkill, float3 point)
        {
            ...
            var Rotation = quaternion.LookRotation(point - nowpos,new float3(0,1,0));
        ...
        public static void PlayNextSkillStep(this SpellComponent self,int index)
        {
            ...
            self.TimerId = TimerComponent.Instance.NewOnceTimer(
                TimeHelper.ServerNow() + self.GetComponent<SkillPara>().GetSkillStepPara(index-1).Interval, TimerInvokeType.PlayNextSkillStep, self);
        }
