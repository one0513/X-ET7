...
namespace ET
{
    ...
    {
        protected override void Awake(CombatUnitComponent self,List<int> skills)
        {
            ...
            EventSystem.Instance.Publish(self.DomainScene(),new EventType.AfterCombatUnitComponentCreate
            {
    ...
    {
        protected override void Awake(CombatUnitComponent self)
        {
            self.AddComponent<SpellComponent>();//技能施法组件
            self.AddComponent<BuffComponent>();//buff容器组件
            self.AddComponent<MoveAndSpellComponent>();//移动过去施法组件
    ...
    {
        protected override void Destroy(CombatUnitComponent self)
        {
