...
namespace ET
{
    [Invoke(TimerInvokeType.SkillColliderRemove)]
    ...
    [Invoke(TimerInvokeType.GenerateSkillCollider)]
    public class GenerateSkillCollider: ATimer<SkillColliderComponent>
    {
        protected override void Run(SkillColliderComponent self)
        {
            try
            {
    ...
    {
        protected override void Awake(SkillColliderComponent self, SkillPara a)
        {
            self.Awake(a);
        }
    }
    ...
    {
        protected override void Awake(SkillColliderComponent self, SkillPara a, long b)
        {
            self.ToId = b;
            self.Awake(a);
        }
    ...
    {
        protected override void Destroy(SkillColliderComponent self)
        {
    ...
    [FriendOf(typeof(SkillColliderComponent))]
    ...
    public static class SkillColliderComponentSystem
    {
        ...
        private static void OnCreate(this SkillColliderComponent self)
        {
            #region 添加触发器
                     ...
                     self.Config.ColliderShape == ColliderShape.OBB)
            {
                ...
                {
                    self.GenerateSkillColliderTimer = TimerComponent.Instance.NewOnceTimer(self.CreateColliderTime, TimerInvokeType.GenerateSkillCollider, self);
                }
            }
            else
            {
            ...
            self.SkillColliderRemoveTimer = TimerComponent.Instance.NewOnceTimer(self.CreateViewTime + self.Config.Time,
                TimerInvokeType.SkillColliderRemove, self.Unit);
        }
        ...
        public static void GenerateSkillCollider(this SkillColliderComponent self)
        {
            ...
            if (self.Config.ColliderShape == ColliderShape.OBB)
            {
                ...
                skillAOIUnit.AddOBBTrigger(par, AOITriggerType.All,
                    (o, e) =>
                    {
                        EventSystem.Instance.Publish(self.DomainScene(),new EventType.OnSkillTrigger
                        {
                            Skill = skillAOIUnit,
                            From = aoiUnit,
                            To = o,
