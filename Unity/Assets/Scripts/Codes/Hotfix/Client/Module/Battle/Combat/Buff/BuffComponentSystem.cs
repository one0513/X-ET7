    ...
    public static class BuffComponentSystem
    {
        ...
        public static void Init(this BuffComponent self,List<int> buffIds,List<long> buffTimestamps,List<long> sourceIds)
        {
            ...
            for (int i = 0; i < buffIds.Count; i++)
            {
                ...
                EventSystem.Instance.Publish(self.DomainScene(),new EventType.AfterAddBuff(){Buff = buff});
            }
        }
        ...
        public static Buff AddBuff(this BuffComponent self, int id,long timestamp,long sourceId)
        {
            ...
            for (int i = 0; i < self.AllBuff.Count; i++)
            {
                ...
                for (int j = 0; j < basebuff.Config.Type.Count; j++)
                {
        ...
        public static bool Remove(this BuffComponent self, long id,bool force = false)
        {
            ...
            if (canRemove||force)
            {
                ...
                EventSystem.Instance.Publish(self.DomainScene(),new EventType.AfterRemoveBuff() { Buff = buff });
                buff.Dispose();
                return true;
            }
            return false;
        ...
        public static void BeforeDamage(this BuffComponent self, Unit attacker,Unit target,DamageInfo damage)
        {
            for (int i = 0; i < self.AllBuff.Count; i++)
            {
                ...
                for (int j = 0; j < buff.Config.Type.Count; j++)
                {
        ...
        public static void ShowAllBuffView(this BuffComponent self)
        {
            ...
            {
                EventSystem.Instance.Publish(self.DomainScene(),new EventType.AfterAddBuff(){Buff = self.GetChild<Buff>(item.Value)});
            }
        }
        ...
        public static void HideAllBuffView(this BuffComponent self)
        {
            foreach (var item in self.Groups)
            {
                EventSystem.Instance.Publish(self.DomainScene(),new EventType.AfterRemoveBuff(){Buff = self.GetChild<Buff>(item.Value)});
            }
        }
#endif
    }
