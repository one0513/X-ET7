...
namespace ET
{
    [FriendOf(typeof(BuffWatcherComponent))]
    public static class BuffWatcherComponentSystem
    {
        ...
        {
            protected override void Awake(BuffWatcherComponent self)
            {
        ...
        {
            protected override void Load(BuffWatcherComponent self)
            {
                self.Init();
            }
        }
        ...
        private static void Init(this BuffWatcherComponent self)
        {
            ...
            HashSet<Type> types = EventSystem.Instance.GetTypes(TypeInfo<ActionControlActiveWatcherAttribute>.Type);
            ...
            types = EventSystem.Instance.GetTypes(TypeInfo<BuffDamageWatcherAttribute>.Type);
            foreach (Type type in types)
            {
            ...
            types =  EventSystem.Instance.GetTypes(TypeInfo<AddBuffWatcherAttribute>.Type);
            foreach (Type type in types)
            {
            ...
            types =  EventSystem.Instance.GetTypes(TypeInfo<RemoveBuffWatcherAttribute>.Type);
            foreach (Type type in types)
            {
            ...
            types =  EventSystem.Instance.GetTypes(TypeInfo<MoveBuffWatcherAttribute>.Type);
            foreach (Type type in types)
            {
        ...
        public static void AfterMove(this BuffWatcherComponent self, int type,Unit target,Buff buff,float3 before)
        {
