    ...
    public static class AOIHelper
    {
            ...
            int gridLen, int xMin, int yMin,float sqrRadius)
        {
            ...
            if (point.x <= xMin) //圆心在格子左方
            {
                ...
                {
                    if ((point - new float2(xMin, yMin)).SqrMagnitude() > sqrRadius)
                        return -1;
                }
                else
                {
                    ...
                    {
                        if ((point - new float2(xMin, yMax)).SqrMagnitude() > sqrRadius)
                            return -1;
                    }
                    else //圆心在格子左侧方
                    {
            ...
            else
            {
                ...
                if (point.x >= xMax) //圆心在格子右方
                {
                    ...
                    {
                        if ((point-new float2(xMax, yMax)).SqrMagnitude() > sqrRadius)
                            return -1;
                    }
                    else if (point.y < yMin) //圆心在格子右下方
                    {
                        if ((point- new float2(xMax, yMin)).SqrMagnitude() > sqrRadius)
                            return -1;
                    }
                    else //圆心在格子右侧方
                    {
        ...
        {
            float3 temp = math.mul(math.inverse(rot),pos - center);
            var xMax = scale.x / 2;
            var yMax = scale.y / 2;
            var zMax = scale.z / 2;
